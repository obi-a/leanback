...
The method
 Couchdb.add_finder(:database => 'contacts', :key => 'email') 
Creates a new design document in the contacts database called _design/email_finder 
...
The method
 Couchdb.find_by( :database => 'contacts', :email => 'nancy@mail.com')  
will query the view we created above, with a
 GET http://127.0.0.1:5984/contacts/_design/email_finder/_view/find_by_email?key="nancy@mail.com"
Leanback parses the results to return only the data values.
